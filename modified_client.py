...
class CAN(object):
    def __init__(self):
        self.db = cantools.database.load_file('../../DBC/honda_civic_sedan_16_diesel_2019_can_generated.dbc')
    ...
    def send_gear(self, gear):
        if gear is "P":
            data = self.gear_message.encode({'GEAR_SHIFTER': 4, 'GEAR': 17})
        elif gear is "R":
            data = self.gear_message.encode({'GEAR_SHIFTER': 8, 'GEAR': 18})
        elif gear is "D":
            data = self.gear_message.encode({'GEAR_SHIFTER': 32, 'GEAR': 20})
